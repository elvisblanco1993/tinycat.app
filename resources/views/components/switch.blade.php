...
<div x-data="{ label: '{{ $label }}', isChecked: @entangle($attributes->wire('model')) }" x-switch:group class="flex items-center">
    <!-- Button -->
    <button x-cloak x-switch x-model="isChecked" :class="isChecked ? 'cursor-pointer bg-blue-500 dark:bg-blue-500' : 'cursor-pointer bg-zinc-300 dark:bg-zinc-500'"
            class="relative inline-flex w-8 rounded-full py-1 transition outline-hidden">

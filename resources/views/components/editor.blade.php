<div x-data="setupEditor(
    ...
    <div class="flex w-full border border-slate-300 border-b-0 divide-x divide-slate-300 bg-slate-100 dark:bg-slate-900 rounded-t-lg">
        <button type="button" class="flex justify-center p-2 transition dark:hover:bg-slate-700 w-10 rounded-tl-md"
            @click="toggleBold();">
            <x-svg class="w-4 h-auto" svg="bold" />
        </button>
        <button type="button" class="flex justify-center p-2 transition dark:hover:bg-slate-700 w-10 dark:bg-slate-900"
            @click="toggleItalic()">

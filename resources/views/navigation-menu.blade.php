<nav x-data="{ open: false }" class="text-zinc-800 dark:text-white">
        ...
        <div class="flex justify-between h-12">
            ...
            <div class="hidden space-x-6 sm:-my-px sm:ms-10 sm:flex">
                ...
                @unless (Auth::user()->is_client)
                    ...
                    <x-nav-link href="{{ route('form.index') }}" :active="request()->routeIs('form.*')">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5">
                ...
                @else
                    <x-nav-link href="{{ route('file.index', ['client' => Auth::user()->ownedClient]) }}" :active="request()->routeIs('file.index')">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5">
                            <path d="M3.75 3A1.75 1.75 0 0 0 2 4.75v3.26a3.235 3.235 0 0 1 1.75-.51h12.5c.644 0 1.245.188 1.75.51V6.75A1.75 1.75 0 0 0 16.25 5h-4.836a.25.25 0 0 1-.177-.073L9.823 3.513A1.75 1.75 0 0 0 8.586 3H3.75ZM3.75 9A1.75 1.75 0 0 0 2 10.75v4.5c0 .966.784 1.75 1.75 1.75h12.5A1.75 1.75 0 0 0 18 15.25v-4.5A1.75 1.75 0 0 0 16.25 9H3.75Z" />
                        </svg>
                        <span>{{ __("Files") }}</span>
                    </x-nav-link>
                    <x-nav-link href="{{ route('upload-request.index', ['client' => Auth::user()->ownedClient]) }}" :active="request()->routeIs('upload-request.*')">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5">
                            <path d="M12.232 4.232a2.5 2.5 0 0 1 3.536 3.536l-1.225 1.224a.75.75 0 0 0 1.061 1.06l1.224-1.224a4 4 0 0 0-5.656-5.656l-3 3a4 4 0 0 0 .225 5.865.75.75 0 0 0 .977-1.138 2.5 2.5 0 0 1-.142-3.667l3-3Z" />
                            <path d="M11.603 7.963a.75.75 0 0 0-.977 1.138 2.5 2.5 0 0 1 .142 3.667l-3 3a2.5 2.5 0 0 1-3.536-3.536l1.225-1.224a.75.75 0 0 0-1.061-1.06l-1.224 1.224a4 4 0 1 0 5.656 5.656l3-3a4 4 0 0 0-.225-5.865Z" />
                        </svg>
                        <span>{{ __("Upload requests") }}</span>
                    </x-nav-link>
                    <x-nav-link href="{{ route('my.task.index') }}" :active="request()->routeIs('my.task.index')">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5">
                            <path d="M5.127 3.502 5.25 3.5h9.5c.041 0 .082 0 .123.002A2.251 2.251 0 0 0 12.75 2h-5.5a2.25 2.25 0 0 0-2.123 1.502ZM1 10.25A2.25 2.25 0 0 1 3.25 8h13.5A2.25 2.25 0 0 1 19 10.25v5.5A2.25 2.25 0 0 1 16.75 18H3.25A2.25 2.25 0 0 1 1 15.75v-5.5ZM3.25 6.5c-.04 0-.082 0-.123.002A2.25 2.25 0 0 1 5.25 5h9.5c.98 0 1.814.627 2.123 1.502a3.819 3.819 0 0 0-.123-.002H3.25Z" />
                        </svg>
                        <span>{{ __("Tasks") }}</span>
                    ...
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            ...
                            @else
                                <span class="inline-flex rounded-md">
                                    <button type="button" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-zinc-500 bg-white hover:text-zinc-700 focus:outline-none focus:bg-zinc-50 active:bg-zinc-50 transition ease-in-out duration-150">
                                        {{ Auth::user()->name }}
                        ...
                        <x-slot name="content">
                            ...
                            @if (Laravel\Jetstream\Jetstream::hasTeamFeatures())
                                @unless (Auth::user()->is_client)
                                    <x-dropdown-link href="{{ route('teams.show', ['team' => Auth::user()->currentTeam]) }}">
                                        {{ __('Team') }}
                                    </x-dropdown-link>
                                @endunless
                            @endif
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            {{-- <x-responsive-nav-link href="{{ route('lead.index') }}" :active="request()->routeIs('lead.index')">
                {{ __('Leads') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link href="{{ route('slides.index') }}" :active="request()->routeIs('slides.index')">
                {{ __('Slides') }}
            </x-responsive-nav-link> --}}
        </div>
...
{{-- <nav class="hidden fixed md:block md:h-full md:w-72 bg-zinc-100 dark:bg-zinc-800 md:py-6 px-4 sm:px-6 border-r dark:border-r-zinc-700">
    ...
    <div class="hidden md:block mt-6 font-medium text-zinc-500">
        ...
        @unless (Auth::user()->is_client)
            ...
            <x-sidenav-link href="{{ route('project.index') }}" :active="request()->routeIs('project.*')">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5">
                    <path d="M5.127 3.502 5.25 3.5h9.5c.041 0 .082 0 .123.002A2.251 2.251 0 0 0 12.75 2h-5.5a2.25 2.25 0 0 0-2.123 1.502ZM1 10.25A2.25 2.25 0 0 1 3.25 8h13.5A2.25 2.25 0 0 1 19 10.25v5.5A2.25 2.25 0 0 1 16.75 18H3.25A2.25 2.25 0 0 1 1 15.75v-5.5ZM3.25 6.5c-.04 0-.082 0-.123.002A2.25 2.25 0 0 1 5.25 5h9.5c.98 0 1.814.627 2.123 1.502a3.819 3.819 0 0 0-.123-.002H3.25Z" />
                </svg>
                <span>{{ __("Projects") }}</span>
        ...
        @else
            <x-sidenav-link href="{{ route('file.index', ['client' => Auth::user()->ownedClient]) }}" :active="request()->routeIs('file.index')">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5">
                    <path d="M3.75 3A1.75 1.75 0 0 0 2 4.75v3.26a3.235 3.235 0 0 1 1.75-.51h12.5c.644 0 1.245.188 1.75.51V6.75A1.75 1.75 0 0 0 16.25 5h-4.836a.25.25 0 0 1-.177-.073L9.823 3.513A1.75 1.75 0 0 0 8.586 3H3.75ZM3.75 9A1.75 1.75 0 0 0 2 10.75v4.5c0 .966.784 1.75 1.75 1.75h12.5A1.75 1.75 0 0 0 18 15.25v-4.5A1.75 1.75 0 0 0 16.25 9H3.75Z" />
                </svg>
                <span>{{ __("Files") }}</span>
            </x-sidenav-link>
            <x-sidenav-link href="{{ route('upload-request.index', ['client' => Auth::user()->ownedClient]) }}" :active="request()->routeIs('upload-request.*')">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5">
                    <path d="M12.232 4.232a2.5 2.5 0 0 1 3.536 3.536l-1.225 1.224a.75.75 0 0 0 1.061 1.06l1.224-1.224a4 4 0 0 0-5.656-5.656l-3 3a4 4 0 0 0 .225 5.865.75.75 0 0 0 .977-1.138 2.5 2.5 0 0 1-.142-3.667l3-3Z" />
                    <path d="M11.603 7.963a.75.75 0 0 0-.977 1.138 2.5 2.5 0 0 1 .142 3.667l-3 3a2.5 2.5 0 0 1-3.536-3.536l1.225-1.224a.75.75 0 0 0-1.061-1.06l-1.224 1.224a4 4 0 1 0 5.656 5.656l3-3a4 4 0 0 0-.225-5.865Z" />
                </svg>
            ...
            <x-sidenav-link href="{{ route('my.task.index') }}" :active="request()->routeIs('my.task.index')">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5">
                    <path d="M5.127 3.502 5.25 3.5h9.5c.041 0 .082 0 .123.002A2.251 2.251 0 0 0 12.75 2h-5.5a2.25 2.25 0 0 0-2.123 1.502ZM1 10.25A2.25 2.25 0 0 1 3.25 8h13.5A2.25 2.25 0 0 1 19 10.25v5.5A2.25 2.25 0 0 1 16.75 18H3.25A2.25 2.25 0 0 1 1 15.75v-5.5ZM3.25 6.5c-.04 0-.082 0-.123.002A2.25 2.25 0 0 1 5.25 5h9.5c.98 0 1.814.627 2.123 1.502a3.819 3.819 0 0 0-.123-.002H3.25Z" />
                </svg>
                <span>{{ __("Tasks") }}</span>
